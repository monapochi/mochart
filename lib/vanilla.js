...
function createChart(container, options) {
  ...
  const publishViewportStateToCtrl = (nextFlags, nextPanOffsetPx, nextGpuDirty, nextHudDirty) => {
  ...
  const scheduleWake = (gpuDirty, hudDirty) => {
    ...
    requestAnimationFrame(() => {
      ...
      const nextGpuDirty = pendingGpuDirty || startBar !== prevStartBar || visibleBars !== prevVisibleBars || currentPlotW !== prevPlotW || currentPlotH !== prevPlotH || currentFlags !== prevFlags || currentPanOffsetPx !== prevPanOffsetPx;
      ...
      publishViewportStateToCtrl(currentFlags, currentPanOffsetPx, nextGpuDirty, nextHudDirty);
  ...
  root.addEventListener("pointermove", (event) => {
    ...
    if (touchGesture === "pan") {
      ...
      if (panByPixelDelta(prevMidX - currMidX)) {
...
//# debugId=A0D17A56059BB94B64756E2164756E21

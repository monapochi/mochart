  ...
  if (typeof Response === "function" && module instanceof Response) {
    if (typeof WebAssembly.instantiateStreaming === "function") {
      try {
        ...
        if (validResponse && module.headers.get("Content-Type") !== "application/wasm") {} else {
          throw e;
        }
      }
    }
...
async function __wbg_init(module_or_path) {
  ...
  if (module_or_path === undefined) {
    module_or_path = new URL("mochart_wasm_bg.wasm", import.meta.url);
  }
  const imports = __wbg_get_imports();
...
var MochartEngineFinalization, WasmColumnarStoreFinalization, WasmRingBufferFinalization, cachedDataViewMemory0 = null, cachedFloat32ArrayMemory0 = null, cachedFloat64ArrayMemory0 = null, cachedUint32ArrayMemory0 = null, cachedUint8ArrayMemory0 = null, heap, heap_next, cachedTextDecoder, MAX_SAFARI_DECODE_BYTES = 2146435072, numBytesDecoded = 0, cachedTextEncoder, WASM_VECTOR_LEN = 0, wasmModule, wasm;
var init_mochart_wasm = __esm(() => {
  ...
  MochartEngineFinalization = typeof FinalizationRegistry === "undefined" ? { register: () => {}, unregister: () => {} } : new FinalizationRegistry((ptr) => wasm.__wbg_mochartengine_free(ptr >>> 0, 1));
  WasmColumnarStoreFinalization = typeof FinalizationRegistry === "undefined" ? { register: () => {}, unregister: () => {} } : new FinalizationRegistry((ptr) => wasm.__wbg_wasmcolumnarstore_free(ptr >>> 0, 1));
  WasmRingBufferFinalization = typeof FinalizationRegistry === "undefined" ? { register: () => {}, unregister: () => {} } : new FinalizationRegistry((ptr) => wasm.__wbg_wasmringbuffer_free(ptr >>> 0, 1));
...
class ChartCore {
  ...
  createRenderer(canvas) {
    ...
    if (backend === "canvas-worker") {
      throw new Error('[Mochart] renderer: "canvas-worker" has been removed. ' + "Use the `worker` option with a Unified Worker for WebGPU rendering instead.");
    }
...
class WorkerBridge {
  ...
  bindInteraction() {
    ...
    gpu.addEventListener("wheel", (e) => {
      ...
      const nextVisible = Math.max(MIN_VISIBLE_BARS2, Math.min(Math.max(MIN_VISIBLE_BARS2, this.totalBars || currentVisible || DEFAULT_VISIBLE_BARS), Math.round(currentVisible * Math.exp(e.deltaY * zoomCoeff))));
  ...
  clampViewport() {
    const maxVisible = Math.max(MIN_VISIBLE_BARS2, this.totalBars || DEFAULT_VISIBLE_BARS);
    this.visibleBars = Math.max(MIN_VISIBLE_BARS2, Math.min(this.visibleBars | 0, maxVisible));
  ...
  zoomAt(factor, centerX) {
    ...
    const currentVisible = this.visibleBars;
    const nextVisible = Math.max(MIN_VISIBLE_BARS2, Math.min(Math.max(MIN_VISIBLE_BARS2, this.totalBars || DEFAULT_VISIBLE_BARS), Math.round(currentVisible * factor)));
    if (nextVisible === currentVisible)
      return;
    const chartAreaWidth = Math.max(1, this.plotW - 60);
  ...
  setViewport(startBar, visBars) {
    ...
    this.visibleBars = Math.max(MIN_VISIBLE_BARS2, visBars);
    this.clampViewport();
    this.scheduleWake(true, true);
  }
  resize() {
...
function createWorkerChart(container, options) {
  let _indicatorHandleCounter2 = 1;
  let _annotationIdCounter2 = 1;
  ...
  function nextAnnotationId2() {
    return `ann_${(_annotationIdCounter2++).toString(36)}`;
  }
  function makeAnnotationHandle(ann) {
    const id = ann.id ?? nextAnnotationId2();
    const stored = { ...ann, id };
  ...
  const api = {
    ...
    addIndicator(config) {
      const handleId = _indicatorHandleCounter2++;
...
function createChart(container, options) {
  ...
  if (options?.worker) {
    options.worker.addEventListener("message", (evt) => {
      const data = evt.data;

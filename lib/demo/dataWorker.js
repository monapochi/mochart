...
var WORKER_BUILD_VERSION = "20260311a";
var WASM_GLUE_VERSION = "20260311a";
...
function _refreshWasmViews() {
  ...
  if (buf !== _lastWasmBuf) {
    ...
    _sharedFdbView = _sharedWasmCapability?.enabled && buf instanceof SharedArrayBuffer && _fdbBase > 0 ? new DataView(buf, _fdbBase, FBUF_HDR_BYTES) : null;
  }
}
...
self.onmessage = async (evt) => {
  ...
  if (_workerInitState === 2) {
    if (totalBars > 0)
      self.postMessage({ type: "ready", bars: totalBars, arenaF32Count: plan ? plan.arena_len() : 0, fdbBase: _fdbBase, wasmMemory: _getSharedWasmBuffer() });
    return;
  }
...
//# debugId=17F9BDAE73A33E0664756E2164756E21

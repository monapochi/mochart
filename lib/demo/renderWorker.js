...
var FBUF_TICK_SIZE = 68;
var FBUF_BASE_PRICE = 72;
  ...
  _drawIndicatorCmds(plotW, candleW, priceMin, priceMax, paneLayout, flags = 4294967295, visBc, offsetSlots = 0) {
    ...
    for (let ci = 0;ci < cmdCount; ci++) {
      ...
      if (style === STYLE_HISTOGRAM) {
        ...
        this._packThickLineUni(dv, plotW, paneH, candleW, yMin, yMax, arenaOff, barCount, warmup, lineW * DPR, cr, cg, cb, ca, visBc, offsetSlots);
...
var _sharedWasmFdbBase = 0;
var _sharedWasmMemory = null;
var _sharedOpenPtr = -1;
var _sharedHighPtr = -1;
var _sharedLowPtr = -1;
var _sharedClosePtr = -1;
var _sharedVolPtr = -1;
var _sharedTimePtr = -1;
var _sharedViewCap = 0;
var _fbOpen;
var _fbHigh;
var _fbLow;
var _fbClose;
...
function _refreshSharedFrameViews(viewLen) {
  ...
  const changed = _fbOpen == null || openPtr !== _sharedOpenPtr || highPtr !== _sharedHighPtr || lowPtr !== _sharedLowPtr || closePtr !== _sharedClosePtr || volPtr !== _sharedVolPtr || timePtr !== _sharedTimePtr || requiredCap > _sharedViewCap;
  if (!changed)
    return true;
  if (openPtr === 0 || highPtr === 0 || lowPtr === 0 || closePtr === 0 || volPtr === 0 || timePtr === 0)
    return false;
...
self.onmessage = async (evt) => {
  if (evt.data.type === "set_shared_wasm_memory") {
    ...
    if (!(nextWasmMemory instanceof SharedArrayBuffer) || nextFdbBase === 0)
      return;
  ...
  frameBuf = fsBuf;
  fdbHdr = _sharedWasmMemory && _sharedWasmFdbBase > 0 ? new DataView(_sharedWasmMemory, _sharedWasmFdbBase, FBUF_HDR_BYTES) : new DataView(fsBuf, 0, FBUF_HDR_BYTES);
...
//# debugId=8023A4670597534764756E2164756E21
